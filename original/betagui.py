...
##Tunes
pvfreqX=epics.PV('TUNEZRP:measX')               #in kHz1
...
## RF frequency
pvfrfSet=epics.PV('MCLKHGP:setFrq')             #in kHz
## Sextupoles setting
...
## orbit, feedback, phase modulation
...
##    alpha0=cal_alpha0()
 ...
 ##   alpha0=0.0265685828781   #call for the function#
    ...
    npoints=int(InputVar[1].get()) #Number of data points Q = Q(delta) or freqX = freqX(frf)
    orderfit=int(InputVar[4].get())
    delt_xmax=float(InputVar[3].get())/1000           #the toleranted maximum displacement of beam position in x direction due to delt_rf#
    delt_xmin=float(InputVar[2].get())/1000         #the tolernated maximun displacement of beam position in -x direction due to delt_f#
    if epics.PV('MLSOPCCP:actOptRmpTblSet').get()==1:
        Dmax=1.5            #the maximum dispersion in the machine#
    ...
    delayChangefrf=float(InputVar[5].get()) #2... Time (seconds) after changing frf
    nmeasurements=int(InputVar[0].get())   #Number of tune measurements - average
    delayMeasTune=float(InputVar[6].get())   #0.8... Time (seconds) after each tune measurement
...
##        newWindow.tkraise()  # <- Personally never needed this, but it's in
        ...
        def start_poly(obj,obj2): 
            ...
            for i in range(len( Sextlist)):  #len( Sextlist)#
                if mea_poly_status and runD:
                    ini_cur=Sextlist[i][0].get()
                    SI=[ini_sext[0],ini_sext[2],ini_sext[3],ini_sext[5]]
                    fitpoints=int(paras[i][1].get())-int(paras[i][0].get()) +1#3 when in debugging#
                    print fitpoints
