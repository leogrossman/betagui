...
## QPD / beam-size signal sources

The GUI includes presets for the likely signal names:
...
These names are placeholders based on your description. Verify exact PVs with EDM/Phoebus panels or `cainfo`.

Purpose:

- use `P1` when microbunching / harmonic signal is available
- use `QPD00/QPD01 SigmaX/SigmaY` as beam-response proxy when microbunching is not on
- optimize laser/electron overlap by minimizing or maximizing the selected signal depending on physics objective
