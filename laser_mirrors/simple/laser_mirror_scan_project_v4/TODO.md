...
## New commissioning TODOs from 50 µm / STOP issue

- [ ] Clarify units in the GUI: motor step, µrad, mm, and µm are not interchangeable.
- [ ] Add optional beam displacement scan mode in µm, explicitly converted through geometry.
- [ ] Add per-axis effective step-to-beam-displacement calibration from measurement.
- [ ] Add "pause after each point" commissioning mode.
- [ ] Add "dry-run CSV export" button for command review.
- [ ] Add configurable hard-stop behavior:
  - graceful stop only
  - hard STOP all
  - never issue .STOP from scan tab
- [ ] Investigate IOC crash when .STOP is sent during motor activity.


## Final deployment polish

- [x] Live trace uses the selected signal source, not only P1.
- [x] Angle and spiral maps use the selected signal source, not only P1.
- [x] Add motor diagnostic button and export.
- [ ] Rename CSV column `p1` to `signal_value` after downstream scripts are updated.
- [ ] Verify QPD actual PV names; current names are presets/placeholders.
- [ ] Add objective direction per signal:
  - P1: maximize
  - SigmaX/SigmaY: clarify whether maximize/minimize depending on machine study.
