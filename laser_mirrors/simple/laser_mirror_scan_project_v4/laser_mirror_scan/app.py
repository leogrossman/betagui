...
class LaserMirrorScanApp:
    ...
    def _build_overview(self):
        ...
        ttk.Button(row, text="Full motor diagnostics", command=self._full_motor_diagnostics).pack(side="left", padx=6)
        ttk.Button(row, text="Export diagnostics", command=self._export_motor_diagnostics).pack(side="left", padx=6)

        self.diag_text = tk.Text(left, width=95, height=10)
        self.diag_text.pack(fill="x", pady=(6, 6))
        ...
        self.signal_live = tk.StringVar(value="simulated")
        ...
        ttk.Label(p1, textvariable=self.signal_live).grid(row=2, column=1, sticky="w")
    ...
    def _build_angle(self):
        ...
        plot = ttk.LabelFrame(self.angle_tab, text="Live 2D selected-signal map", padding=10)
    ...
    def _build_spiral(self):
        ...
        controls.grid(row=0, column=0, sticky="nsew")
        plot = ttk.LabelFrame(self.spiral_tab, text="Selected signal over mirror 2 position", padding=10)
        plot.grid(row=0, column=1, sticky="nsew", padx=(12, 0))
    ...
    def _motor_diag_fields(self):
        return [
            "VAL", "RBV", "DMOV", "MOVN", "STOP", "DESC", "EGU", "STAT", "SEVR", "RTYP",
            "PREC", "HOPR", "LOPR", "VELO", "VBAS", "ACCL", "BDST", "BVEL",
            "HLM", "LLM", "DHLM", "DLLM", "MRES", "ERES", "SREV", "UREV",
            "RVAL", "DVAL", "OFF", "DIR", "SET", "LVIO", "HLS", "LLS", "MSTA",
        ]

    def _read_motor_field_safe(self, base: str, field: str):
        try:
            pv = self.factory.pv(base + "." + field, "")
            return pv.get(timeout=0.35)
        except Exception as exc:
            return f"<ERR {type(exc).__name__}: {exc}>"

    def _collect_motor_diagnostics(self):
        data = {
            "timestamp": now(),
            "safe_mode": self.safe_mode,
            "write_mode": self.write_mode,
        ...
        fields = self._motor_diag_fields()
        for key, motor in self.motors.items():
            rec = {"base": motor.base, "fields": {}}
            for field in fields:
                rec["fields"][field] = self._read_motor_field_safe(motor.base, field)
            data["motors"][key] = rec
        return data

    def _format_motor_diagnostics(self, data) -> str:
        lines = [f"Motor diagnostics @ {data['timestamp']}"]
        for key, rec in data["motors"].items():
            lines.append("")
            lines.append(f"=== {key}  {rec['base']} ===")
            for field, value in rec["fields"].items():
                lines.append(f"{field:>6s}: {value}")
        return "\n".join(lines)

    def _full_motor_diagnostics(self):
        data = self._collect_motor_diagnostics()
        text = self._format_motor_diagnostics(data)
        self.diag_text.delete("1.0", "end")
        self.diag_text.insert("end", text)
        self._log("FULL_MOTOR_DIAGNOSTICS\n" + text)

    def _export_motor_diagnostics(self):
        data = self._collect_motor_diagnostics()
        path = self.session_dir / ("motor_diagnostics_" + now().replace(":", "").replace(".", "_") + ".json")
        path.write_text(json.dumps(data, indent=2, sort_keys=True, default=str), encoding="utf-8")
        text_path = path.with_suffix(".txt")
        text_path.write_text(self._format_motor_diagnostics(data), encoding="utf-8")
        self._log(f"MOTOR_DIAGNOSTICS_EXPORTED json={path} txt={text_path}")
    ...
    def _connect_p1(self):
        ...
        if not name:
            ...
            return
        self.signal_pv = self.factory.pv(name, 0)
        self._log(f"P1 connected {name}")

    def _read_p1(self, ah=0.0, av=0.0) -> float:
        if self.signal_pv is not None:
            try:
                return float(self.signal_pv.get(timeout=0.5))
            except Exception:
                return math.nan
    ...
    def _update_p1(self):
        ...
        self.signal_live.set(f"{self.signal_kind.get()}: {val:.5g}" if val == val else f"{self.signal_kind.get()}: nan")
    ...
    def _scan_worker(self, plan: list[ScanPoint], csv_path: Path, dwell: float, samples: int, status_var, draw_func):
        ...
        with csv_path.open("w", newline="", encoding="utf-8") as f:
            ...
            for p in plan:
                ...
                self._log(f"MEAS point={p.index} signal_kind={self.signal_kind.get()} pv={self.p1_name.get()} value={p1:.6g}")
    ...
    def _draw_trace(self):
        ...
        c.delete("all")
        c.create_text(10, 10, anchor="w", text="Selected signal trace")
    ...
    def _draw_map(self, canvas, plan, measurements, xattr, yattr, xlabel, ylabel):
        ...
        margin = 55
        c.create_text(10, 10, anchor="w", text=f"{self.signal_kind.get()} map: {xlabel} vs {ylabel}")
