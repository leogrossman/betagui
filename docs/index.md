# betagui Documentation

This is the documentation landing page for the MLS `betagui` Python 3 port.

## For Control-Room Users

- [overview.md](overview.md)
- [runtime_checklist.md](runtime_checklist.md)
- [control_room_test_plan.md](control_room_test_plan.md)
- [feature_parity.md](feature_parity.md)
- [theory_and_operation.md](theory_and_operation.md)
- [write_paths.md](write_paths.md)

## For Developers

- [development_workflow.md](development_workflow.md)
- [testing_workflow.md](testing_workflow.md)
- [open_features.md](open_features.md)
- [gitlab_docs_plan.md](gitlab_docs_plan.md)
- [reference_context.md](reference_context.md)

## For Code Understanding

- [theory_and_operation.md](theory_and_operation.md)
- [code_walkthrough.md](code_walkthrough.md)
- [porting_notes.md](porting_notes.md)
- [pv_inventory.md](pv_inventory.md)

## Suggested GitLab Start Page

If you want a repo homepage for testers, point them first to:

1. [overview.md](overview.md)
2. [runtime_checklist.md](runtime_checklist.md)
3. [feature_parity.md](feature_parity.md)

If you later move this material into a GitLab Wiki or Pages site, this file is a
good starting table of contents.

## Source Repository

Source files, workflows, and root-level project files live in the repository:

- <https://github.com/leogrossman/betagui>
